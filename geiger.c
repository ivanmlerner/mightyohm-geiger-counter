/*
  ...
  ***** WARNING *****
  ...
  Change log:
  08/04/11 1.00: Initial release for Chaos Camp 2011!
  24/10/23 1.10: Add average measure and long press to reset.
...
ISR(TIMER1_COMPA_vect) {
  ...
  fastcpm = 0;
  for(i=0; i<SHORT_PERIOD;i++) {                   // sum up the last 5 CPS values
    int8_t x = idx - i;
    if (x < 0)
      x = LONG_PERIOD + x;
    fastcpm += buffer[x];
...
void uart_putstring(char *buffer) {       // start sending characters over the serial port until we reach the end of the string
...
void uart_putstring_P(char *buffer) {       // start sending characters over the serial port until we reach the end of the string
...
void checkevent(void) {
  if (eventflag) {              // a GM event has occurred, do something about it!
    ...
    _delay_ms(10);              // 10ms delay gives a nice short flash and 'click' on the piezo
...
int main(void) {
  ...
  while(1) {                          // Configure AVR for sleep, this saves a couple mA when idle
